...
for DFName in DFNames:
    ...
    ax.set_color_cycle([cm(1.*i/Num_Individuals) for i in range(Num_Individuals)])
...
#     # Clustering
#     # silhouetteScore = {}
#     # for n_clusters in range(10,35,5): 
#     model = DBSCAN(eps=3, min_samples=10, n_jobs = 7)
#     # eps=0.5, min_samples=5 --> silhouette score: -0.386; eps=3, min_samples=5 --> silhouette score: 0.718;
#     # eps=3, min_samples=10 --> silhouette score: 0.845
#     model.fit(DF_Scaled)
#     # centroids = model.core_sample_indices_
#     print('Average silhouette score: ', silhouette_score(X=DF_Scaled, labels=model.labels_, random_state=42))
#     
#     counteredLabels = Counter(model.labels_)
#     
...
#     # for n_clusters in range(10,35,5): 
#     n_clusters = 10
#     model = agc(n_clusters=n_clusters)
#     model.fit(DF_Scaled)
#     print('Average silhouette score: ', silhouette_score(X=DF_Scaled, labels=model.labels_, random_state=42))
#         
#     descaled_centroids = (centroids*scaler.scale_ + scaler.mean_)
...
#     # for n_clusters in range(10,35,5): 
#     n_clusters = 30
#     model = SC(n_clusters=n_clusters, n_jobs=7, random_state=42)
#     model.fit(DF_Scaled)
#     print('Average silhouette score: ', silhouette_score(X=DF_Scaled, labels=model.labels_, random_state=42))
#         
#     descaled_centroids = (centroids*scaler.scale_ + scaler.mean_)
